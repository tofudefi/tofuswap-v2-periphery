...
import '@tofudefi/tofuswap-v2-core/contracts/interfaces/ITofuswapV2Callee.sol';
...
contract ExampleFlashSwap is ITofuswapV2Callee {
    ...
    constructor(address _factory, address _factoryV1, address router) public {
        ...
        factory = _factory;
        WTRX = IWTRX(ITofuswapV2Router01(router).WTRX());
    }
    ...
    function tofuswapV2Call(address sender, uint amount0, uint amount1, bytes calldata data) external {
        ...
        address token0 = ITofuswapV2Pair(msg.sender).token0();
        address token1 = ITofuswapV2Pair(msg.sender).token1();
        assert(msg.sender == TofuswapV2Library.pairFor(factory, token0, token1)); // ensure that msg.sender is actually a V2 pair
        ...
        ITofuswapV1Exchange exchangeV1 = ITofuswapV1Exchange(factoryV1.getExchange(address(token))); // get V1 exchange

        if (amountToken > 0) {
            (uint minTRX) = abi.decode(data, (uint)); // slippage parameter for V1, passed in by caller
